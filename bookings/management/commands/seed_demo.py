...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        random.seed(42)
        username, password = settings.DEMO_LOGIN_USERNAME, settings.DEMO_LOGIN_PASSWORD
        user, _ = User.objects.get_or_create(username=username, defaults={"first_name":"Alex","last_name":"Morgan","email":"demo@turfiq.local"})
        user.email = "demo@turfiq.local"
        user.is_active = True
        user.set_password(password)
        user.save()
        Subscription.objects.update_or_create(owner=user, defaults={"status": "active", "current_end": None})
        ...
        self.stdout.write(self.style.SUCCESS(f"Demo ready with full access: username {username} / password {password}"))
