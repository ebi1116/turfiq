...
def build_dashboard(user):
    ...
    qs=Booking.objects.filter(owner=user).exclude(status="Cancelled"); expenses=Expense.objects.filter(owner=user)
    ...
    completed=Booking.objects.filter(owner=user,status="Completed").count(); cancelled=Booking.objects.filter(owner=user,status="Cancelled").count(); count=Booking.objects.filter(owner=user).count()
    customer_counts=Booking.objects.filter(owner=user).values("customer").annotate(n=Count("id")); returning=customer_counts.filter(n__gt=1).count(); customers=customer_counts.count()
    ...
    booked=float(qs.filter(booking_date__range=(month_start,today)).aggregate(v=Sum("duration"))["v"] or 0); available=work_hours*max(today.day,1); occupancy=min(booked/available*100,100)
    cards=[
        ...
        ("Total Bookings",count,0,"all bookings","calendar-check"), ("Completed",completed,0,"successful bookings","circle-check"),
